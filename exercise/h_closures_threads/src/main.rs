...
fn main() {
    ...
    let t1 = thread::spawn(move || { 
        println!("Received signal {:?} from Sender 3", rx3.recv().unwrap())
    });
    let t2 = thread::spawn(move || {
        println!("Received singal {:?} from Sender 4", rx4.recv().unwrap())
    });
    let result  = t2.join().expect("sth went wrong");
    let result2 = t1.join().unwrap();
}
